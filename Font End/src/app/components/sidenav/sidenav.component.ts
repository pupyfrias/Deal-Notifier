...
export class sidenavComponent implements OnInit {
  ...
  public brandsList: string[] = ["samsung", "iphone", "lg", "huawei", "alcatel", "motorola", "xiaomi", "htc"];
  public storagesList: string[] = ["32", "64", "128", "256", "512"];
  public typesList: Array<any> = [["phone", '1'], ["memory", '3'], ["tv", '2'], ["streaming", '7'],
  ["headphone", '4'], ["speaker", '5'], ["microphone", '6'], ["accesory", '8']];
  public carriersList: string[] = ['unlocked', 'boost', 'cricket', 'verizon', 'at&t', 't-mobile',
    'consumer cellular', 'us cellular'];
  public shopsList: Array<any> = [['Amazon', '1'], ['eBay', '2'], ['TheStore', '3']]
    ...
    this.router.events.subscribe(
      event => {
        if (event instanceof NavigationStart) {
          ...
          const { url } = event;
          this.queryParams = new URLSearchParams(decodeURIComponent(url).replace('/', ''));
          this.brandsQuery = this.queryParams.get('brands') != null ? this.queryParams.get('brands')?.split(',')! : [];
          this.storagesQuery = this.queryParams.get('storages') != null ? this.queryParams.get('storages')?.split(',')! : [];
          this.shopsQuery = this.queryParams.get('shops') != null ? this.queryParams.get('shops')?.split(',')! : [];
          this.conditionsQuery = this.queryParams.get('condition') != null ? this.queryParams.get('condition')?.split(',')! : [];
          this.typesQuery = this.queryParams.get('types') != null ? this.queryParams.get('types')?.split(',')! : [];
          this.carriersQuery = this.queryParams.get('carriers') != null ? this.queryParams.get('carriers')?.split(',')! : [];
          this.excludesQuery = this.queryParams.get('excludes') != null ? this.queryParams.get('excludes')?.split(',')! : [];
          this.maxPrice = this.queryParams.get('max') != null ? this.queryParams.get('max')! : '';
          this.minPrice = this.queryParams.get('min') != null ? this.queryParams.get('min')! : '';
  ...
  //#region ToTitleCase
  public ToTitleCase(value: any): string {
    if (value.length <= 3) {
  ...
  settingQueryParams(params: string, checked: boolean, category: string[]): void {

    if (checked === true) { category.push(params) }
    else { this.DeleteElement(category, params) }
  }
  //#endregion
  ...
  checkboxChange(e: any): void {
    ...
    if (e.id === "consumer cellular") {
      ...
      this.settingQueryParams('consumer%2Bcellular', e.checked, this.carriersQuery);
    }
    if (e.id === "us cellular") {
  ...
  SettingsNavigation() {
    ...
    let queryJson;
    const listParams = [[this.brandsQuery, 'brands'], [this.storagesQuery, 'storages'],
    [this.conditionsQuery, 'condition'], [this.typesQuery, 'types'], [this.carriersQuery, 'carriers'],
    [this.excludesQuery, 'excludes'], [this.shopsQuery, 'shops']];

    for (let i = 0; i < listParams.length; i++) {
      const category = listParams[i][0];
      const nameCategory = listParams[i][1].toString();

      if (category.length > 0) { this.queryParams.append(nameCategory, category.toString()) }
      else { this.queryParams.delete(nameCategory) }
    }

    //QUERY JSON
    if (this.queryParams.toString() !== "") {
      queryJson = JSON.parse('{"' + decodeURI(this.queryParams.toString()
        .replace(/&/g, "\",\"").replace(/=/g, "\":\"")) + '"}')
    }
    else {
      queryJson = {}
